...
<script>
import { TutorialApiService } from "../services/tutorial-api.service";
...
export default {
  ...
  created() {
    this.tutorialsService = new TutorialApiService();
    ...
    saveTutorial() {
      ...
      if (this.tutorial.title.trim()) {
        if (this.tutorial.id) {
          ...
          this.tutorial = this.getStorableTutorial(this.tutorial);
          this.tutorialsService.created(this.tutorial).then((response) => {
